...
function astarPath(nodes, adj, startId, goalId) {
    ...
    const openSet = [startId];          // searched node
    const cameFrom = {};
    ...
    while (openSet.length > 0) {
        ...
        for (const nb of (adj[current] || [])) {
            ...
            if (!currNode || !nextNode) continue;
                const cost = Math.hypot(currNode.x - nextNode.x, currNode.z - nextNode.z);  // calcolate the real destince 

            const tentativeG = (gScore[current] ?? Infinity) + cost;
            if (tentativeG < (gScore[neighbor] ?? Infinity)) {               // *************** If 'New f' < 'odl f' update path **************************
